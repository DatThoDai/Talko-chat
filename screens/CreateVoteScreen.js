...
const handleSubmit = async () => {
  ...
  try {
    // Lọc bỏ các tùy chọn trống và định dạng đúng format API cần
    const validOptions = options
      .filter(option => option.trim() !== '')
      .map(option => ({
        name: option.trim(), // Sử dụng name thay vì content
        userIds: [] // Khởi tạo mảng rỗng cho userIds
      }));
    ...
    if (response && response.data) {
      ...
      const newVoteMessage = {
        ...response.data,
        sender: user, // Thêm thông tin người gửi
        isMyMessage: true,
        forceMyMessage: true
      };
